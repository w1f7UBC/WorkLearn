...
import com.AROMA.Logistics.Delay;
...
import com.AROMA.Logistics.Fleet;
...
public class FacilityTransportationManager extends FacilityManager {
	...
	public void scheduleTransportation(Contract contract,double amount){
		...
		// if transportation is not modeled for this contract, tele port material from one stock pile to the other
			...
			assignedAmount = Tester.min(amount, supplierPile.getCurrentlyHandlingAmount(),
			...
			contract.getSupplier().addToStocksList(contract.getProduct(), 16, assignedAmount);
				
			contract.getBuyer().addToStocksList(contract.getProduct(), 15, assignedAmount);
